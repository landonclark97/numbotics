...
class PRMStar(SamplingPlannerBase):
    ...
    def connection_radius(self):
        ...
        V_ball = (np.pi**(dim / 2.0)) / scipy.special.gamma((dim / 2.0) + 1.0)
        V_space = self._space.volume
        gamma = 2.0 * (1.0 + (1.0 / dim)) ** (1.0 / dim) * (V_ball / V_space) ** (1.0 / dim)
        return gamma * (np.log(k) / k) ** (1 / dim)

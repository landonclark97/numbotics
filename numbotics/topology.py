...
def dist_from_bb(bb, w, b, qs, signed=False, bb_index=False, weights=None):
    ...
    qw_minus_b = np.stack((qw_minus_b[...,np.newaxis], qw_minus_b[...,np.newaxis]),-1)
    ...
    if signed:
        ...
        signed_diffs = np.squeeze(np.take_along_axis(bb_diffs, signed_inds[...,None], axis=-1), axis=-1)

        which_diff = np.argmin(np.linalg.norm(signed_diffs, axis=-1), axis=1)[:, np.newaxis, np.newaxis]
        which_inds = np.broadcast_to(which_diff, (signed_diffs.shape[0], 1, signed_diffs.shape[2]))
        signed_diffs = np.take_along_axis(signed_diffs, which_inds, axis=1)[:,0,:]

    bb_diffs = np.amin(np.abs(bb_diffs), axis=-1)

    if weights is not None:
        assert ((weights.shape==(bb_diffs.shape[-1],)) or (weights.shape==(bb_diffs.shape[0],bb_diffs.shape[-1])))
    ...
    min_indices = np.argmin(np.tile(bb_norm[...,np.newaxis],(1,1,bb_diffs.shape[-1])), axis=1)

    bb_diffs = np.take_along_axis(bb_diffs, np.expand_dims(min_indices,axis=1),axis=1).squeeze(1)

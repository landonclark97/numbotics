...
gfx = (
    None  # imported if graphics are used - starts viz window even if imported otherwise
)
...
class Robot:
    def __init__(self, filename=None, links=None, base=np.identity(4), params=None):
            ...
            for l in range(self.n):
                self.links.append(
                    Link(
                        rob_params[l + 1, :],
                        (
                            rob_params[self.n + (2 * l) + 1 : self.n + (2 * l) + 3, :]
                            if self.use_dyn
                            else None
                        ),
                    )
        ...
        if self.use_gfx:
            ...
            gfx.inst_gfx(xlim=[-1.3, 1.3], ylim=[-1.3, 1.3], zlim=[-1.3, 1.3])
    ...
    def jac(self):
        ...
        ind = np.concatenate(
            (
                np.arange(self.pos_dof, dtype=int),
                np.arange(6 - self.orn_dof, 6, dtype=int),
            ),
            dtype=int,
            casting="unsafe",
        )
    ...
    def hess(self):
        ...
        H[0:3, i, j] = np.cross(
            J[3:6, np.minimum(i, j)].T, J[0:3, np.maximum(i, j)].T
        ).T
    ...
    def min_post_fail_sing(self):
        ...
        J_f = np.swapaxes(
            np.reshape(np.swapaxes(J[:, rem_list], 0, 1), (self.n, self.n - 1, self.m)),
            1,
            2,
        )
    ...
    def d_sigma(self, J, H):
        ...
        for i in range(self.m):
            for k in range(self.n):
                grad[i, k] = (
                    U.T[i, np.newaxis, :] @ H[:, :, k] @ V[i, np.newaxis, :].T
                )[0, 0]
        return grad
    ...
    def _jac_com(self, end_frame=None):
        ...
        for i, l in enumerate(self.links[:end_frame]):
            fk_mats[i + 1, :, :] = fk_mats[i, :, :] @ l.forward()
        fk_mats[end_frame + 1, :, :] = (
            fk_mats[end_frame, :, :] @ self.links[end_frame].forward_com()
        )
        on = fk_mats[end_frame + 1, 0:3, 3]
        for i, l in enumerate(self.links[: end_frame + 1]):
            zi = fk_mats[i, 0:3, 2]
    ...
    def batch_fk_err(self, x_d, b_q, mask=True):
        ...
        if mask:
            dx_mask = [
                True if (i < self.pos_dof or i >= 6 - self.orn_dof) else False
                for i in range(6)
            ]
    ...
    def batch_jac(self, b_q, mask=True):
        ...
        if isinstance(b_q, np.ndarray):
            ...
            for i, l in enumerate(self.links):
                ...
                if l.link_type == 1:
                    J[:, 0:3, i] = np.cross(
                        ret_mat[:, i, 0:3, 2], (on - ret_mat[:, i, 0:3, 3])
                    )
    ...
    def batch_hess(self, b_q):
        ...
        if isinstance(b_q, np.ndarray):
            ...
            H[:, 0:3, i, j] = np.swapaxes(
                np.cross(
                    np.swapaxes(J[:, 3:6, np.minimum(i, j)], 1, 2),
                    np.swapaxes(J[:, 0:3, np.maximum(i, j)], 1, 2),
                ),
                1,
                2,
            )
            H[:, 3:6, i, j] = np.swapaxes(
                np.cross(
                    np.swapaxes(J[:, 3:6, j], 1, 2), np.swapaxes(J[:, 3:6, i], 1, 2)
                ),
                1,
                2,
            )
            ...
            with torch.no_grad():
                ...
                H[:, 0:3, i, j] = torch.transpose(
                    torch.cross(
                        torch.transpose(J[:, 3:6, torch.minimum(i, j)], 1, 2),
                        torch.transpose(J[:, 0:3, torch.maximum(i, j)], 1, 2),
                    ),
                    1,
                    2,
                )
                H[:, 3:6, i, j] = torch.transpose(
                    torch.cross(
                        torch.transpose(J[:, 3:6, j], 1, 2),
                        torch.transpose(J[:, 3:6, i], 1, 2),
                    ),
                    1,
                    2,
                )
    ...
    def batch_d_sigma(self, Js, Hs):
        assert (Js.shape[1:] == (self.m, self.n)) and (
            Hs.shape[1:] == (6, self.n, self.n)
        )
        ...
        if isinstance(Js, np.ndarray):
            ...
            H = np.hstack(
                (Hs[:, 0 : self.pos_dof, :, :], Hs[:, 6 - self.orn_dof : 6, :, :])
            )  # don't overwrite inputs
            grad = np.zeros((B, self.m, self.n))
            for i in dims:
                for k in range(self.n):
                    grad[:, i, k] = (
                        np.swapaxes(U, 1, 2)[:, i, np.newaxis, :]
                        @ H[:, :, :, k]
                        @ np.swapaxes(V[:, i, np.newaxis, :], 1, 2)
                    )[:, 0, 0]
            ...
            with torch.no_grad():
                ...
                for i in dims:
                    for k in range(self.n):
                        grad[:, i, k] = (
                            torch.transpose(U, 1, 2)[:, i, np.newaxis, :]
                            @ H[:, :, :, k]
                            @ torch.transpose(V[:, i, np.newaxis, :], 1, 2)
                        )[:, 0, 0]
    ...
    def batch_ik(self, x_d, b_q=None, ik_iters=500, dls_lambda=0.1, thresh=1e-8):
        ...
        if isinstance(x_d, np.ndarray):
            ...
            for i in range(ik_iters):
                dx = self.batch_fk_err(x_d[~success], b_q[~success])

                J = self.batch_jac(b_q[~success])
                # Jt = np.swapaxes(J, 1, 2)
                # dq = (Jt @ np.linalg.inv((J @ Jt) + (b_dls_lambdas[~success] * b_I[~success]))) @ dx

                # b_q[~success] += dq.squeeze(2) * dt

                dq = np.linalg.pinv(J) @ dx
                b_q[~success] += dq.squeeze(2) * 0.1
                dx_new = self.batch_fk_err(x_d[~success], b_q[~success], mask=True)
                ...
                # b_dls_lambdas[~success][low_i, 0, 0] /= 2.0
                # b_dls_lambdas[~success][high_i, 0, 0] *= 2.0

                # b_dls_lambdas = np.clip(b_dls_lambdas, 1e-6, dls_lambda)

                subset_indices = np.where(~success)[0]
                success[subset_indices[np.where(new_err < thresh)]] = True
                if np.all(success):
                    break
    ...
    def batch_min_post_fail_sing(self, b_q, ignore_j=None):
        ...
        Js_f = np.swapaxes(
            np.reshape(
                np.swapaxes(Js[:, :, rem_list], 1, 2), (-1, self.n, self.n - 1, self.m)
            ),
            2,
            3,
        )

        sings = np.linalg.svd(Js_f, compute_uv=False)[
            :, [i for i in range(self.n) if i not in ignore_j], :
        ]
        ...
        for iters in range(ik_iters):
            ...
            if new_err < err:
                ...
                if rej_cnt >= rej_limit:
                    nlog.warning(
                        "too many consecutive rejected IK steps - hit limit: "
                        + str(rej_limit)
                    )
                    break
    ...
    def all_smms(
        ...
        while current_without < without_new:
            ...
            if s:
                ...
                for mf in manifold_data:
                    if adapt_diff:
                        diff_thresh = 2.0 * np.mean(
                            np.linalg.norm(tr_inv(mf[:, :-1] / mf[:, 1:]), axis=1)
                        )
                    if (
                        np.amin(np.linalg.norm(tr_inv(mf.T / tr(self.q.T)), axis=1))
                        < diff_thresh
                    ):
                        add_manifold = False
                        current_without += 1
                ...
                if s_smm == 1:
                    ang = tr(angles)
                    current_without = 0
                    manifold_data = np.concatenate(
                        (manifold_data, np.swapaxes(ang, 0, 1)[np.newaxis, ...]), axis=0
                    )

                elif s_smm == 2:
                    manifold_data = np.zeros(
                        (max_manifs + 1, self.n, samples, 0), dtype=np.complex128
                    )
                    success = 2
        ...
        if (manifold_data.shape[0] != 0) and manifold_data.shape[0] <= max_manifs:
            ...
            if plot:
                ...
                fig = plt.figure()
                axs = [
                    fig.add_subplot(plots, 1, i + 1, projection="3d")
    ...
    def sim(self, T, dynamics="nonlinear", sol=nsol.ARK23(), u_func=None, disp=False):
        ...
        if dynamics == "linearized":
            ...
            state_mat[self.n : self.n * 2, self.n : self.n * 2] = -2.0 * np.identity(
                self.n
            )
    ...
    def sim_zoh(
        ...
        T,
        sol=nsol.ARK23(),
        u_func=lambda x, t: np.zeros((x.shape[0], 1)),
        control_freq=100.0,
        disp=False,
    ):
        if disp:
...
if __name__ == "__main__":
    ...
    r.sim_zoh(
        np.inf, sol=nsol.ARK23(auto_inc=True), u_func=u, control_freq=10.0, disp=True
    )
